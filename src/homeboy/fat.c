...
static int cache_flush(fat_ctxt_t *fat, enum fat_cache_type type){
    ...
    if(!sdio_write_sectors(cache->load_lba, cache->sector_cnt, cache->data)){
        errno = EIO;
        return -1;
    }
    cache->dirty = 0;
...
static void *cache_prep(fat_ctxt_t *fat, enum fat_cache_type type, uint32_t lba, _Bool load){
    ...
    if(load){
        ...
        if(!sdio_read_sectors(lba, cache->sector_cnt, cache->data)){
            cache->valid = 0;
            errno = EIO;
            return NULL;
        }
...
static void *cache_load(fat_ctxt_t *fat, enum fat_cache_type type, uint32_t lba){
    ...
    sdio_read_sectors(lba, cache->sector_cnt, cache->data);
    cache->valid = 1;
    cache->dirty = 0;
    cache->load_lba = lba;
    cache->prep_lba = lba;
...
static uint32_t cluster_rw(fat_file_t *file, enum fat_io rw, void *buf, uint32_t cluster_cnt, _Bool *eof){
    ...
    while(cluster_cnt > 0){
        ...
        int e;
        if(rw == FAT_READ){
            e = sdio_read_sectors(lba, block_cnt, p);
        }else{
            e = sdio_write_sectors(lba, block_cnt, p);
        }
        if(!e){
            break;
        }
